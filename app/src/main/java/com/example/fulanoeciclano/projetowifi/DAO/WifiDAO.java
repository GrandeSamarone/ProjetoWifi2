package com.example.fulanoeciclano.projetowifi.DAO;
...
public class WifiDAO extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String sql = "CREATE TABLE db_wifi (id,INTEGER PRIMARY KEY,nome_do_wifi TEXT NOT NULL," +
                "senha_do_wifi DOUBLE NOT NULL)";
        db.execSQL(sql);
    ...
    public void onUpgrade(SQLiteDatabase db, int antigaversao, int novaversao) {
        String sql = "DROP TABLE IF EXISTS db_wifi AND db_anuncio ";
        db.execSQL(sql);
        onCreate(db);
    ...
    /*public void insereanuncio(String titulo_do_anuncio_selecionado,String texto_do_anuncio_selecionado){
        SQLiteDatabase dbanuncio = getWritableDatabase();

        ContentValues dadosAnuncio = new ContentValues();
        dadosAnuncio.put("titulo_do_anuncio",titulo_do_anuncio_selecionado);
        dadosAnuncio.put("texto_do_anuncio",texto_do_anuncio_selecionado);

        dbanuncio.insert("db_anuncio",null,dadosAnuncio);
